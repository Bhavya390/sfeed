...
public function postfetch($admin_id,$ide1)
{
	$query = $this->db->prepare("SELECT * FROM posts WHERE Id = '$admin_id' && pos_id = '$ide1'");
try{
		$query->execute();
		$data=$query->fetch();
		return $data;
...
public function addpost($content,$admin_id)
{	
	$query = $this->db->prepare("INSERT INTO `posts`( `Id`,  `PostCont`, `PostDate`) VALUES ('$admin_id','$content','now()')");
	try{
...
public function score($home_name,$home_score,$opp_name,$opp_score,$admin_id)
{	
	$query = $this->db->prepare("INSERT INTO `score`(`Id`, `TeamAscore`,`TeamAname`,`TeamBscore`,`TeamBname`,`Dateandtime`) VALUES ('$admin_id','$home_score','$home_name','$opp_score','$opp_name',now())");
	try{
...
public function deletescore($id,$admin)
{
	$query = $this->db->prepare("DELETE FROM score WHERE score_id ='$id' and Id ='$admin'");
	try{
		return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
...
public function scoretext($admin,$score_id)
{
	$query = $this->db->prepare("SELECT * FROM `score` WHERE  Id = '$admin' and score_id = '$score_id'");
	try{
		$query->execute();
		return $query->fetch();
	}catch(PDOException $e){
...
public function blogselect($admin,$comment_id)
{
	$query = $this->db->prepare("SELECT * FROM `comments` WHERE  Id = '$admin' and comment_id = '$comment_id'");
	try{
		$query->execute();
		return $query->fetch();
	}catch(PDOException $e){
...
public function updatescore($hname,$hscore,$oname,$oscore,$id,$postid)
{
	$query = $this->db->prepare("UPDATE `score` SET `TeamAscore`='$hscore',`TeamAname`='$hname',`TeamBscore`='$oscore',`TeamBname`='$oname'  WHERE `score_id`='$postid' and `Id` = '$id'");
	try{
		return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
...
public function updatemess($id,$pos,$con)
{
	$query = $this->db->prepare("UPDATE `comments` SET `messages`='$con' , `postdate` =now() WHERE `comment_id`='$pos' and `Id`='$id' ");
	try{
		return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
...
public function deletemember($id,$admin)
{
	$query = $this->db->prepare("DELETE FROM member WHERE id_member1 ='$id' and Id ='$admin'");
	try{
		return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
...
public function delete($po_id,$admin)
{
	$query = $this->db->prepare("DELETE FROM posts WHERE pos_id ='$po_id' and Id='$admin'");
	try{
		return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
...
{
	$query = $this->db->prepare("SELECT * FROM `posts` WHERE pos_id = '$pos_id'");
	try{
		$query->execute();
		return $query->fetch();
	}catch(PDOException $e){
...
public function updatemembers($id,$membername,$rollno,$dept,$postion,$aboutme,$image)
{
	$query = $this->db->prepare("UPDATE `member` SET `membername`='$membername',`rollno`='$rollno',`dept`='$dept',`pos`='$postion' ,`aboutme`='$aboutme',`file_name`='$image' ,`post_date`='date()'  WHERE `id_member1`='$id'");
	try{
		return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
...
public function addmember($admin_id,$name,$rollno,$dept,$position,$aboutme,$image)
{
	$query = $this->db->prepare("INSERT INTO `member`( `Id`, `membername`, `rollno`, `dept`, `pos`,`aboutme`,`file_name`,`post_date`) VALUES ('$admin_id','$name','$rollno','$dept','$position','$aboutme','$image' ,'date()')");
try{
	return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
...
public function updatepost($content,$date,$pos_id){

	$query = $this->db->prepare("update posts set  PostCont='$content' WHERE pos_id ='$pos_id' ");
	try{
		return $query->execute();
		}catch(PDOException $e){
			die($e->getMessage());
